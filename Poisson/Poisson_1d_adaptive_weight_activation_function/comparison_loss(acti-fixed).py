...
def Plot_loss():
    loss_adaptive=np.load('./result_data/training_loss2(8000)_(Weighted_Average).npy')
    loss_softmax = np.load('./result_data/training_loss(8000)_(Softmax).npy')
    loss_L1norm = np.load('./result_data/training_loss(8000)_(L1norm).npy')
    loss_L2norm = np.load('./result_data/training_loss(8000)_(L2norm).npy')
    loss_Identity = np.load('./result_data/training_loss(8000)_(Identity).npy')
    loss_Sigmoid = np.load('./result_data/training_loss(8000)_(Sigmoid).npy')
    loss_fixed=np.load('./result_data/training_loss(8000)_(Fixed).npy')
    plt.cla()
    ...
    plt.plot(loss_Identity, label='PINN with Identity',
             color='c', linestyle='-', lw=1.2,zorder=2)
    ...
    plt.plot(loss_L1norm, label='PINN with L1norm',
             color='lightcoral', linestyle='-', lw=1.2,zorder=2.1)
    # plt.plot(loss_L2norm, label='PINN with L2norm',
    #          color='darkorchid', linestyle='-', lw=1.2,zorder=4)
    # plt.plot(loss_adaptive,label='PINN with Weighted Average',
    #          color='forestgreen',linestyle='-',lw=1.2,zorder=4.2)
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.yscale('log')
    plt.legend(loc='best',prop = {'size':8})
    # plt.savefig('./result_plot/loss_comparison_{}(all).png'.format(8000),bbox_inches='tight',dpi=600,format='png',
    #             pad_inches=0.1)
    plt.savefig('./result_plot/defenses{}(all).png'.format(8000),bbox_inches='tight',dpi=600,format='png',
                pad_inches=0.1)
    plt.close()
...
def time_comparison():
    t_adap=np.load('./result_data/train-time(8000)_(Weighted_Average).npy')
    t_L2norm = np.load('./result_data/train-time(8000)_(L2norm).npy')
    t_Softmax = np.load('./result_data/train-time(8000)_(Softmax).npy')
    t_Identity = np.load('./result_data/train-time(8000)_(Identity).npy')
    t_L1norm = np.load('./result_data/train-time(8000)_(L1norm).npy')
    t_Sigmoid = np.load('./result_data/train-time(8000)_(Sigmoid).npy')
    t_fixed = np.load('./result_data/train-time(8000)_(Fixed).npy')
...
def Plot_Weights():
    epochs=8000
    a_history = np.load('./result_data/sin-weight_8000(L2norm).npy')
    b_history = np.load('./result_data/tanh-weight_8000(L2norm).npy')
    c_history = np.load('./result_data/GELU-weight_8000(L2norm).npy')
    d_history = np.load('./result_data/Swish-weight_8000(L2norm).npy')
    e_history = np.load('./result_data/Softplus-weight_8000(L2norm).npy')
    ...
    plt.legend(bbox_to_anchor=(0.94,0.75))
...
def Plot_AF():
    ...
    epochs = 8000
    a_history_weighted = torch.tensor(np.load('./result_data/sin-weight_8000(Weighted_Average).npy'))
    b_history_weighted = torch.tensor(np.load('./result_data/tanh-weight_8000(Weighted_Average).npy'))
    c_history_weighted = torch.tensor(np.load('./result_data/GELU-weight_8000(Weighted_Average).npy'))
    d_history_weighted= torch.tensor(np.load('./result_data/Swish-weight_8000(Weighted_Average).npy'))
    e_history_weighted = torch.tensor(np.load('./result_data/Softplus-weight_8000(Weighted_Average).npy'))

    a_history_L2norm = torch.tensor(np.load('./result_data/sin-weight_8000(L2norm).npy'))
    b_history_L2norm = torch.tensor(np.load('./result_data/tanh-weight_8000(L2norm).npy'))
    c_history_L2norm = torch.tensor(np.load('./result_data/GELU-weight_8000(L2norm).npy'))
    d_history_L2norm = torch.tensor(np.load('./result_data/Swish-weight_8000(L2norm).npy'))
    e_history_L2norm = torch.tensor(np.load('./result_data/Softplus-weight_8000(L2norm).npy'))

    a_history_Softmax = torch.tensor(np.load('./result_data/sin-weight_8000(Softmax).npy'))
    b_history_Softmax = torch.tensor(np.load('./result_data/tanh-weight_8000(Softmax).npy'))
    c_history_Softmax = torch.tensor(np.load('./result_data/GELU-weight_8000(Softmax).npy'))
    d_history_Softmax = torch.tensor(np.load('./result_data/Swish-weight_8000(Softmax).npy'))
    e_history_Softmax = torch.tensor(np.load('./result_data/Softplus-weight_8000(Softmax).npy'))

    a_history_Sigmoid = torch.tensor(np.load('./result_data/sin-weight_8000(Sigmoid).npy'))
    b_history_Sigmoid = torch.tensor(np.load('./result_data/tanh-weight_8000(Sigmoid).npy'))
    c_history_Sigmoid = torch.tensor(np.load('./result_data/GELU-weight_8000(Sigmoid).npy'))
    d_history_Sigmoid = torch.tensor(np.load('./result_data/Swish-weight_8000(Sigmoid).npy'))
    e_history_Sigmoid = torch.tensor(np.load('./result_data/Softplus-weight_8000(Sigmoid).npy'))

    a_history_Identity = torch.tensor(np.load('./result_data/sin-weight_8000(Identity).npy'))
    b_history_Identity = torch.tensor(np.load('./result_data/tanh-weight_8000(Identity).npy'))
    c_history_Identity = torch.tensor(np.load('./result_data/GELU-weight_8000(Identity).npy'))
    d_history_Identity = torch.tensor(np.load('./result_data/Swish-weight_8000(Identity).npy'))
    e_history_Identity = torch.tensor(np.load('./result_data/Softplus-weight_8000(Identity).npy'))

    a_history_L1norm = torch.tensor(np.load('./result_data/sin-weight_8000(L1norm).npy'))
    b_history_L1norm = torch.tensor(np.load('./result_data/tanh-weight_8000(L1norm).npy'))
    c_history_L1norm = torch.tensor(np.load('./result_data/GELU-weight_8000(L1norm).npy'))
    d_history_L1norm = torch.tensor(np.load('./result_data/Swish-weight_8000(L1norm).npy'))
    e_history_L1norm = torch.tensor(np.load('./result_data/Softplus-weight_8000(L1norm).npy'))
    ...
    plt.legend(loc='best')
    plt.savefig('./result_plot/Final AF1.png', bbox_inches='tight',
                format='png')
    plt.close()
